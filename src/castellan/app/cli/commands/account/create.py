...
parser.add_argument(
    "--account-oobi", "-o", help="out-of-band introduciton to load", required=True
)
parser.add_argument(
    "--account-username",
    help="alias for AID resolved from out-of-band introduciton",
    required=False,
    default=None,
)
...
def create(args):
    ...
    with existing.existingHab(args.alias, args.name, args.base, args.bran) as (
        hby,
        hab,
    ):
        ...
        print(
            f"Account {args.account_username} created successfully with AID {account_aid}"
        )
